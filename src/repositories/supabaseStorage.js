...
async function listLeads(workspaceId) {
  ...
  return supabaseRequest("GET", "leads", `?workspace_id=eq.${encodeURIComponent(ws)}&order=updated_at.desc`);
}
...
async function findUser(workspaceId, email) {
  const ws = workspaceId || "default";
  const users = await supabaseRequest("GET", "app_users", `?workspace_id=eq.${encodeURIComponent(ws)}&email=eq.${encodeURIComponent(email)}&select=*&limit=1`);
  const user = users[0];
  return user
    ? {
        id: user.id,
...
async function createUser(workspaceId, user) {
  ...
  const entry = {
    ...
    password_salt: salt,
    password_hash: passwordHash(user.password || "", salt),
    workspace_id: ws
  };
  const rows = await supabaseRequest("POST", "app_users", "", [entry]);
  ...
  await supabaseRequest("POST", "app_users", "", [
    {
      ...
      role: "admin",
      password_salt: salt,
      password_hash: passwordHash("admin123456", salt),
